...
public class MyAlgoSPADE {
    ...
    public Object[] getFrequentSequences(int patternLen){
    	ArrayList<ArrayList<String>> freqseq = new ArrayList<ArrayList<String>>();
    	ArrayList<Integer> supports = new ArrayList<Integer>();  
    	
    	String allLines = saver.print();
    	...
    	for(int i=0; i<lines.length; i++){
    		...
    		if(!line.matches("^$")){
    			if(line.contains("Level ")){
    				if(line.contains("Level " + patternLen)){
    					isLevelFound = true;
    				} else {
    					if(isLevelFound){
    						break;
    				...
    				if(isLevelFound){
    					Object[] objA = this.getItem1Sequence(line);
    					ArrayList<String> seq = (ArrayList<String>)objA[0];
    					int sup = (int)objA[1];
    					freqseq.add(seq);
    					supports.add(sup);
    				}
    ...
    private Object[] getItem1Sequence(String line){
		String[] seq1 = line.split(" ");
		ArrayList<String> finalseq = new ArrayList<String>();
		int support = 0;
		for(int j=0; j<seq1.length; j++){
			String elem = seq1[j];
			if(elem.contains("#SUP")){
				support = Integer.valueOf(seq1[j+1]).intValue();
				break;
			} else {
				if(!elem.equals("-1")){
					finalseq.add(elem);
