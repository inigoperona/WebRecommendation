...
public class ModelEvaluatorSeqMin 
	...
	protected ArrayList<ArrayList<ArrayList<String[]>>> m_minedSeqs2;
	...
	protected void writeWeightedSequences2(String outfilename){
		// Open the given file
		BufferedWriter writer = null;
		try{
			writer = new BufferedWriter(new FileWriter(outfilename));
		...
		try{
			for(int i=0; i<m_nFolds; i++){
				ArrayList<ArrayList<String[]>> fold = m_minedSeqs2.get(i);
				int nCl = fold.size();
				for(int j=0; j<nCl; j++){
					ArrayList<String[]> cl = fold.get(j);
					int nSeq = cl.size();
					for(int k=0; k<nSeq; k++){
						String line = "fold:" + i + ";cl:" + j + ";";
						String[] seq = cl.get(k);
						line = line + seq[0];
